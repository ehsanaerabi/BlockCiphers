...
void TestVectors(int ver)
{
	...
	for(n = 1; n < 10; n++)
	{
		...
		fprintf(fic,"TK = "); for(i = 0; i < (versions[ver][1]>>3); i++) fprintf(fic,"%02x", k[i]); fprintf(fic,"\n");
		fprintf(fic,"P =  "); for(i = 0; i < (versions[ver][0]>>3); i++) fprintf(fic,"%02x", p[i]); fprintf(fic,"\n");
		enc(c,k,ver);
		fprintf(fic,"C =  "); for(i = 0; i < (versions[ver][0]>>3); i++) fprintf(fic,"%02x", c[i]); fprintf(fic,"\n");
		dec(c,k,ver);
		fprintf(fic,"P' = "); for(i = 0; i < (versions[ver][0]>>3); i++) fprintf(fic,"%02x", c[i]); fprintf(fic,"\n\n");
	}
}
